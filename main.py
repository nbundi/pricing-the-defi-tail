...
def build_sector_tvl_panel(flat_protocols, target_chains: set[str]) -> pd.DataFrame:
    """Daily TVL by sector across all in-scope chains. All sectors use net
    TVL (idle, borrowed excluded): net is the value actually at risk, and is
    the single consistent basis for every capital denominator in the paper.
    Gross supplied TVL (idle + borrowed) is used only to select the top-10
    Lending protocols (so high-utilisation venues like Euler are included)."""
    ...
    for sector, slug, _summary in flat_protocols:
        ...
        try:
            ...
            continue
        s = protocol_chain_tvl(p, target_chains, include_borrowed=False)
        if s.empty:
            continue
